...
def tg_modes(update, context):
    try:
        ...
        update.message.reply_text('формат ввода(исключение ассоциаций): если вплотную перед словом поставить восклицательный знак либо знак процента, то бот не будет брать к этому слову ассоциации. например, "!окно,дом.сарай": бот возьмет для первого списка слова окно, дом и ассоциации к слову дом, для второго списка - слово сарай и ассоциации к слову сарай')
        update.message.reply_text('формат ввода(накуй ассоциации): если первым символом в сообщении поставить занк доллара, то формат ввода будет совсем другой: все слова будут браться не через запятую, а через любые небуквенные символы, а также к этим ассоциациям не будут браться ассоциации. например, "$корова день окно мышь овал" в режиме гибридов вернет один ответ: корОВАл. то есть бот возьмет только те слова которые указаны, без ассоциаций')
